...
@torch.no_grad()
def do_batch(model: CLIP, x, text):
    batch_size = x.size(0)
...
if __name__ == "__main__":
    clip_model = "models/RN50.pt"
    clip_weights = "models/CLIB-FIQA_R50.pth"
    ...
    print(
        f"x: {tensor_data.shape},{tensor_data.dtype}  ;  text: {joint_texts.shape},{joint_texts.dtype}"
    )

    (
        logits_per_image,
        _,
    ) = do_batch(model, tensor_data, joint_texts)
    ...
    logits_per_image_k = logits_per_image.view(
        logits_per_image.size(0),
        -1,
        len(quality_list),
    )
    logits_quality_k = logits_per_image_k.sum(1)
    logits_quality = logits_per_image.sum(1).sum(1).sum(1).sum(1).sum(1)
    print(logits_quality, logits_quality_k)
    logits_blur = (
        torch.max(logits_per_image.sum(6).sum(5).sum(4).sum(3).sum(2), dim=1)[1]
        .cpu()
        .detach()
        .numpy()
        .squeeze(0)
    ...
    logits_occ = (
        torch.max(logits_per_image.sum(6).sum(5).sum(4).sum(3).sum(1), dim=1)[1]
        .cpu()
        .detach()
        .numpy()
        .squeeze(0)
    ...
    logits_pose = (
        torch.max(logits_per_image.sum(6).sum(5).sum(4).sum(2).sum(1), dim=1)[1]
        .cpu()
        .detach()
        .numpy()
        .squeeze(0)
    ...
    logits_exp = (
        torch.max(logits_per_image.sum(6).sum(5).sum(3).sum(2).sum(1), dim=1)[1]
        .cpu()
        .detach()
        .numpy()
        .squeeze(0)
    ...
    logits_ill = (
        torch.max(logits_per_image.sum(6).sum(4).sum(3).sum(2).sum(1), dim=1)[1]
        .cpu()
        .detach()
        .numpy()
        .squeeze(0)
